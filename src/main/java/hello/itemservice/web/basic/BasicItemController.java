...
//    @PostMapping("/add")
    ...
    /**
     * RedirectAttributes를 사용해서 addAttribute해준다면
     * attributeName으로 value값이 들어가게 된다.
     *
     * 들어간 값을 view의 논리이름에 {itemId}
     * 이렇게 해서 사용해줄 수도 있고
     *
     * 논리이름에 들어가지 않은 값은 쿼리 파라미터로 uri에들어간다.
     * ?status=ture 요렇게
     *
     * 결론적으로 리다이렉트 결과 item메서드가 반환되게 된다.
     */
    @PostMapping("/add")
